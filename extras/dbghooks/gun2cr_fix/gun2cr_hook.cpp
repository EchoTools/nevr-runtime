...
int __fastcall hook_InitBulletCI(CR15NetBullet2CS* this_ptr, SR15NetBullet2CI* bullet_instance,
  ...
  // CRITICAL DIAGNOSTIC: Log EVERY call to prove hook is working
  log_to_file("========== HOOK CALLED (type_hash=0x%08X) ==========", component_id.type_hash);
  printf("[Gun2CR] hook_InitBulletCI called! type_hash=0x%08X\n", component_id.type_hash);
