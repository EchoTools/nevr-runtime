...
/* ======================================================================
 * binary_bug_fixes — Bug fix hooks for verified binary bugs
 *
 * Hooks:
...
#include "runtime/ext/plugin_loader.h"    // N68: TickPlugins
...
#include "runtime/lifecycle/crash_recovery.h"   // ForceFatalExit, PerformGracefulShutdown
#endif
...
void BinaryBugFixes::Init(uintptr_t base_addr) {
    ...
    Log(EchoVR::LogLevel::Debug, "[NEVR.PATCH] qpc_freq=%lld Hz",
    ...
    s_cached_timer = CreateWaitableTimerExW(NULL, NULL,
        ...
        TIMER_ALL_ACCESS);
    if (s_cached_timer) {
        Log(EchoVR::LogLevel::Debug, "[NEVR.PATCH] timer=high_resolution");
    } else {
...
void BinaryBugFixes::Shutdown() {
    ...
    if (s_busywait_patched) {
        ...
        if (busywait) {
            ...
            Log(EchoVR::LogLevel::Info,
                "[NEVR.PATCH] binary bug fixes shutdown restored BusyWait byte 0x%02X at va=0x%llX",
                static_cast<unsigned int>(s_busywait_original_byte),
                static_cast<unsigned long long>(VA_PRECISION_SLEEP_BUSYWAIT));
        }
