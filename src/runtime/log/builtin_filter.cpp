...
/* N89 follow-up, second defect: this was called ONLY from inside the CLog hook.
...
void BuiltinLogFilter::InstallPnsradHook() {
    ...
    // native/unfiltered rather than risking the game process for log formatting.
    static bool s_logged = false;
    if (!s_logged) {
        s_logged = true;
        Log(EchoVR::LogLevel::Warning,
            "[NEVR.PATCH] pnsrad CLog hook disabled — retaining native pnsrad output to avoid Wine stack overflow");
    }
}
