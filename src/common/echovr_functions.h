...
/// Registers a callback for a websocket message type.
/// Returns an identifier for unregistering the callback.
...
/// Registers a callback for a broadcast message type.
/// Returns an identifier for unregistering the callback.
...
/// Unregisters a broadcast message callback.
...
/// Gets a JSON string value with a default fallback.
...
/// Path uses pipe separators (e.g. "round_rules|round_time").
...
/// Parses an HTTP/HTTPS URI to be connected to.
typedef UINT64 HttpConnectFunc(VOID* unk, CHAR* uri);
extern HttpConnectFunc* HttpConnect;

/// Loads a JSON file from disk into a Json structure. Returns 0 on success.
...
/// The game's definition for SetWindowTextA.
...
// --- UDP/protocol function declarations (from Ghidra decompilation) ---
