...
struct Lobby {
  ...
  struct EntrantData {
    ...
    Json json;                       // 0x90
    UINT16 slotIndex;                // 0xA0 @ 0x178 from EntrantData base (verified via Ghidra)
    CHAR _unk_cosmetic_data[0x1A8];  // 0xA2 - Hidden cosmetic/loadout data (0x250 total size)
  };
  ...
  HeapArray<Lobby::EntrantData> entrantData;  // 0x360 - Array of players in lobby

  // VERIFIED COSMETIC LOADOUT OFFSETS:
  // Player loadout instances accessible at:
  //   lobby_ptr + 0x51420 + (slotIndex * 0x40)
  // Where slotIndex = EntrantData.slotIndex & 0x1F
  //
  // Array structure at that location:
  //   [0] = ptr to first loadout ID
  //   [6] = count of loadout instances (uint64)
  //
  // Cosmetic data for player resolved via:
  //   LoadoutData = FUN_1404f37a0(*(void**)(lobby_ptr + 0x08), loadout_id)
  //   Primary cosmetic: *(int64*)(*(int64*)(LoadoutData + 0x370 + 0xb8) + (slot_id * 8))
  //   Fallback cosmetic: *(int64*)(*(int64*)(LoadoutData + 0x370 + 0x80) + (slot_id * 8))

  CHAR _unk_after_entrants[0x1C0];  // 0x378 - Unknown data (size estimate)

  // NOTE: The following fields were previously marked as TODOs but are NOT direct Lobby fields.
  // They are stored in related server game context structures (SNSServerContext, CNSLobby,
  // CR15NetGame, etc.) accessed through IServerLib or via callback parameters.
  //
  // VERIFIED SERVER CONTEXT OFFSETS (from LobbySessionSuccessCB @ 0x14017db90):
  //
  // 1. Registration State (int32_t at +0x448):
  //    - Checked as: (*(int*)(serverContext + 0x448) - 4) < 3
  //    - Valid states: 4, 5, 6 (after subtracting 4, becomes 0, 1, 2)
  //    - Invalid states: 0-3 (reserved/error), 7+ (error)
  //
  // 2. Entrant Connections Array (at serverContext + 0x910):
  //    - Base structure accessed as: *(longlong**)(serverContext + 0x910)
  //    - Sub-structure offsets:
  //      +0x50: pointer to EntrantData array (EntrantData**)
  //      +0x58: count of entrants (uint64)
  //      +0x68: matchmaking queue entries (void** array)
  //      +0xF0: lobby type storage (int32_t) - value checked as == 2 in callback
  //      +0x28d0: related data structure pointer
  //
  // 3. Platform Identifier (from RemovingEntrantCB @ 0x1401a0860):
  //    - Location: EntrantData structure + 0x08 (char value)
  //    - Platform values: 7='a'(PS5), 8='b'(Xbox), 9='t'(Steam/PC)
  //    - This is sent in network messages (USNSLobbyRemovingEntrant)
  //
  // 4. Crossplay Enabled (location TBD):
  //    - Referenced in error handling at 0x14017d380
  //    - Related to lobby_type enum value 6 ("lobby no crossplay" error)
  //    - Likely a boolean flag in EntrantConnections structure
  //
  // 5. Lobby Type (int32_t, location partially verified):
  //    - Stored at: EntrantConnections + 0xF0 (based on plVar7[0x1e] = array[0xF0/8])
  //    - Values: 0=Public, 1=Private, 2=Unassigned, 6=NoCrossplay
  //
  // HOOK POINT: IServerLib::RequestRegistration receives:
  //    - serverId (int64)
  //    - radId (char*, registration ID)
  //    - regionId (SymbolId, server platform region symbol)
  //    - lockedVersion (SymbolId, version lock symbol)
  //    - localConfig (Json*)
  //
  // To populate these fields in a Lobby hook, the callback context (serverContext)
  // must be preserved or access maintained to SNSServerContext during gameplay.
  //
  // Note: 0x358 (QWORD) set to 1 will load map instead of load server in some circumstances.
};
