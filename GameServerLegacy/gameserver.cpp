...
VOID SendServerdbTcpMessage(GameServerLib* self, EchoVR::SymbolId msgId, VOID* msg, UINT64 msgSize) {
  ...
  if (self->wsClient) {
    ...
    Log(EchoVR::LogLevel::Error, "[ECHORELAY.GAMESERVER.LEGACY] Cannot send message - WebSocket client not initialized");
  }
}
...
VOID OnTcpMessageStartSession(GameServerLib* self, VOID* proxymthd, EchoVR::TcpPeer sender, VOID* msg, VOID* unk,
  ...
  Log(EchoVR::LogLevel::Info, "[ECHORELAY.GAMESERVER.LEGACY] Starting new session");
...
/// <summary>
/// Event handler for receiving a players accepted message from the TCP
/// (websocket) ServerDB service. This message indicates that ServerDB /
/// Matching services accepted a player into this session, and is now requesting
/// the game server accept them.
/// </summary>
/// <returns>None</returns>
...
/// <summary>
/// Event handler for receiving a players rejected message from the TCP
/// (websocket) ServerDB service. This message indicates that ServerDB /
/// Matching services rejected a player from this session, and is now requesting
/// the game server kick / reject them.
/// </summary>
/// <returns>None</returns>
...
/// parameters for both parties, including encryption / verification keys for
/// client / game server to use.
/// </summary>
/// <returns>None</returns>
...
VOID OnMsgSessionStarting(GameServerLib* self, VOID* proxymthd, VOID* msg, UINT64 msgSize, EchoVR::Peer destination,
  ...
  Log(EchoVR::LogLevel::Info, "[ECHORELAY.GAMESERVER.LEGACY] Session starting");
}
...
VOID OnMsgSessionError(GameServerLib* self, VOID* proxymthd, VOID* msg, UINT64 msgSize, EchoVR::Peer destination,
                       EchoVR::Peer sender) {
  // NOTE: `msg` here has no substance (one uninitialized byte).
  Log(EchoVR::LogLevel::Error, "[ECHORELAY.GAMESERVER.LEGACY] Session error encountered");
}
...
VOID* GameServerLib::Initialize(EchoVR::Lobby* lobby, EchoVR::Broadcaster* broadcaster, VOID* unk2,
  ...
  Log(EchoVR::LogLevel::Info, "[ECHORELAY.GAMESERVER.LEGACY] version %s (%s) initializing", PROJECT_VERSION,
      GIT_COMMIT_HASH);
  ...
  this->wsClient->SetMessageHandler([this](EchoVR::SymbolId msgId, const VOID* data, UINT64 size) {
    // Route the message through the TCP broadcaster's message handling system
    // This allows existing message handlers to work unchanged
    EchoVR::TcpBroadcasterHandlePeerData(this->lobby->tcpBroadcaster, msgId, this->serverDbPeer, data, size);
  });
  ...
  // debugging purposes.
#if _DEBUG
  Log(EchoVR::LogLevel::Debug, "[ECHORELAY.GAMESERVER.LEGACY] EchoVR base address = 0x%p",
      (VOID*)EchoVR::g_GameBaseAddress);
#endif
...
/// <summary>
/// Requests registration of the game server with central TCP/websocket services
/// (ServerDB). This is called by the game after the library has been
/// initialized.
/// </summary>
/// <param name="serverId">The identifier to use for the game server when
/// registering with ServerDB.</param> <param name="radId">TODO:
/// Unknown.</param> <param name="regionId">A 64-bit symbol identifier
/// indicating the region that the game server should be registering to.</param>
/// <param name="versionLock">A version to use when locking out clients which
/// request matching with a differing version.</param> <param
/// name="localConfig">The game config containing service endpoints (located in
/// ./_local/config.json from the root of game folder).</param>
/// <returns>None</returns>
VOID GameServerLib::RequestRegistration(INT64 serverId, CHAR* radId, EchoVR::SymbolId regionId,
  ...
  if (EchoVR::UriContainerParse(&serverDbUriContainer, serverDbServiceUri) != ERROR_SUCCESS) {
    Log(EchoVR::LogLevel::Error,
        "[ECHORELAY.GAMESERVER.LEGACY] Failed to register game server: error parsing "
  ...
  if (!this->wsClient->Connect(serverDbServiceUri)) {
    Log(EchoVR::LogLevel::Error,
        "[ECHORELAY.GAMESERVER.LEGACY] Failed to connect to ServerDB at %s", serverDbServiceUri);
    return;
  }
  ...
  // Log the interaction.
  Log(EchoVR::LogLevel::Info, "[ECHORELAY.GAMESERVER.LEGACY] Requested game server registration");
}
...
VOID GameServerLib::EndSession() {
  ...
  Log(EchoVR::LogLevel::Info, "[ECHORELAY.GAMESERVER.LEGACY] Signaling end of session");
}
...
VOID GameServerLib::LockPlayerSessions() {
  ...
  // Log the interaction.
  Log(EchoVR::LogLevel::Info, "[ECHORELAY.GAMESERVER.LEGACY] Signaling game server locked");
}
...
VOID GameServerLib::UnlockPlayerSessions() {
  ...
  // Log the interaction.
  Log(EchoVR::LogLevel::Info, "[ECHORELAY.GAMESERVER.LEGACY] Signaling game server unlocked");
}

/// <summary>
/// Signals acceptance of player sessions by the game server, with central
/// TCP/websocket services (ServerDB). This is called by the game after a peer
/// connects to the game server, typically after ServerDB signals its acceptance
/// to game server, and gives peer the appropriate information to join.Both
/// parties must have exchanged packet encoding settings via receipt of
/// SNSLobbySessionSuccessv5 from ServerDB / Matching to communicate.
/// </summary>
/// <param name="playerUuids">An array of player session UUIDs which have been
/// accepted by the game server. </param> <returns>None</returns>
VOID GameServerLib::AcceptPlayerSessions(EchoVR::Array<GUID>* playerUuids) {
  ...
  // Log the interaction.
  Log(EchoVR::LogLevel::Info, "[ECHORELAY.GAMESERVER.LEGACY] Accepted %d players into game server", playerUuids->count);
}
...
VOID GameServerLib::RemovePlayerSession(GUID* playerUuid) {
  ...
  // Log the interaction.
  Log(EchoVR::LogLevel::Info, "[ECHORELAY.GAMESERVER.LEGACY] Removed a player from game server");
}
